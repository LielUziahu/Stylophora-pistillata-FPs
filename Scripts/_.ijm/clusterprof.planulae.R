...
go_ont <- AnnotationDbi::select(
  ...
  rename(term = GOID) %>%
  distinct(term, ONTOLOGY)

term2gene_bp_mf <- term2gene %>%
  inner_join(go_ont, by = "term") %>%
  filter(ONTOLOGY %in% c("BP", "MF")) %>%
  select(term, gene) %>%
  distinct()

# also filter TERM2NAME to match
term2name_bp_mf <- term2name %>%
  inner_join(go_ont, by = "term") %>%
  filter(ONTOLOGY %in% c("BP", "MF")) %>%
  select(term, name) %>%
  distinct()
...
enrichment <- enricher(interesting_set_FL,
                       TERM2GENE = term2gene_bp_mf,
                       TERM2NAME = term2name_bp_mf,
                       pvalueCutoff = 0.05,
                       universe = background_genes,
                       pAdjustMethod = "fdr",
                       qvalueCutoff = 0.2)
...
# Save enrichment results
write_csv(enrichment@result, "enrichment_results.bp_mf_FL.csv")
...
# Save enrichment results
write_csv(enrichment@result, "enrichment_results.mf.csv")
...
# Save enrichment results
write_csv(enrichment@result, "enrichment_results.bp.csv")

#### bar plot bp ####

#### bar plot ####
deg_results <- list(
  FLvNF   = read_csv("../res_annot_stranded.csv")
)
